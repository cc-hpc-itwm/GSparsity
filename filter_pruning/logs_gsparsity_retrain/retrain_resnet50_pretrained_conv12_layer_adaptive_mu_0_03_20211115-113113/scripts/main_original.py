...
def main():
    ...
    if args.resume:
        ...
        if args.run_id is None:
            args.run_id = "lr_{}_momentum_{}_wd_{}_{}".format(args.lr,
                                                                 args.momentum,
                                                                 args.weight_decay,
                                                                 time.strftime("%Y%m%d-%H%M%S"))
        else:
            args.run_id = "{}_lr_{}_momentum_{}_wd_{}_{}".format(args.run_id,
                                                                    args.lr,
                                                                    args.momentum,
                                                                    args.weight_decay,
                                                                    time.strftime("%Y%m%d-%H%M%S"))
...
#     print("torch.cuda.is_available is {}, args.distributed is {}, args.gpu is {}, ".format(torch.cuda.is_available(), args.distributed, args.gpu))
    ...
    num_zero_filters, num_total_filters, num_zero_params, num_total_params = print_nonzeros_filters(model, pruning_threshold=1e-6)
    logger.info("    zero/total filters (ratio): {}/{} ({})".format(num_zero_filters, num_total_filters, num_zero_filters/num_total_filters))
    logger.info("    zero/total params (ratio): {}M/{}M ({})".format(num_zero_params/1e6, num_total_params/1e6, num_zero_params/num_total_params))
    
#     num_zero_filters, num_total_filters, num_zero_params, num_total_params = print_nonzeros_filters(model, pruning_threshold=1e-6)
#     num_zero_filters, num_total_filters, num_zero_params, num_total_params = print_nonzeros_filters(model, pruning_threshold=1e-5)
#     num_zero_filters, num_total_filters, num_zero_params, num_total_params = print_nonzeros_filters(model, pruning_threshold=1e-4)
#     num_zero_filters, num_total_filters, num_zero_params, num_total_params = print_nonzeros_filters(model, pruning_threshold=1e-3)
#     num_zero_filters, num_total_filters, num_zero_params, num_total_params = print_nonzeros_filters(model, pruning_threshold=1e-2)
#     num_zero_filters, num_total_filters, num_zero_params, num_total_params = print_nonzeros_filters(model, pruning_threshold=1e-1)
    
    compute_and_save_mask(model, "{}/mask".format(args.path_to_save), pruning_threshold=1e-6)
...
#             print("    value {}".format(filter_norms))
    ...
    print("pruning threshold: {}, zero/total filters (ratio): {}/{} ({})".format(pruning_threshold, num_zero_filters, num_total_filters, num_zero_filters/num_total_filters))
    print("pruning threshold: {},  zero/total params (ratio): {}/{}M ({})".format(pruning_threshold, num_zero_params/1e6, num_total_params/1e6, num_zero_params/num_total_params))
