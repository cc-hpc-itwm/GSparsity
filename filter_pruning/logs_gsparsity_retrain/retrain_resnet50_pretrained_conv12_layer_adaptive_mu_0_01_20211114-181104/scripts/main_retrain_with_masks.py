...
model_names = sorted(name for name in models_with_masks.__dict__
    if name.islower() and not name.startswith("__")
    and callable(models_with_masks.__dict__[name]))
...
#     model = models_with_masks.__dict__[args.arch](mask)        
    if args.pretrained:
        print("=> using pre-trained model '{}'".format(args.arch))
        model = models_with_masks.__dict__[args.arch](mask, pretrained=True)
    else:
        print("=> creating model '{}'".format(args.arch))
        model = models_with_masks.__dict__[args.arch](mask)
...
#     print("torch.cuda.is_available is {}, args.distributed is {}, args.gpu is {}, ".format(torch.cuda.is_available(), args.distributed, args.gpu))
    ...
    if args.path_to_resume:
        ...
        if os.path.isfile(path_to_prune): # This will overwrite the pretrained model loaded earlier
    ...
    num_zero_filters, num_total_filters, num_zero_params, num_total_params = print_nonzeros_filters(model)
