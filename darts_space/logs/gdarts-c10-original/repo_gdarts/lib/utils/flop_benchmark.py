...
def count_parameters(models_or_parameters, unit="mb"):
    if isinstance(models_or_parameters, nn.Module):
        counts = sum(np.prod(v.size()) for v in models_or_parameters.parameters())
    elif isinstance(models_or_parameters, nn.Parameter):
        counts = models_or_parameters.numel()
    elif isinstance(models_or_parameters, (list, tuple)):
        counts = sum(count_parameters(x) for x in models_or_parameters)
    else:
        counts = sum(np.prod(v.size()) for v in models_or_parameters)
