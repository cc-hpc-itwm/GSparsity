...
def visualize_cell_in_network(network, alpha_network, scale_type, folder_path):
  if scale_type == "cell":
    ...
    for cell in network.reduce_cell_indices:
        visualize_cell(alpha_network[cell-1][1], network.steps, network.ops['primitives_normal'], "{}/cell_in_stage_normal_{}".format(folder_path, stage_index))
        visualize_cell(alpha_network[cell][1], network.steps, network.ops['primitives_reduct'], "{}/cell_in_stage_reduce_{}".format(folder_path, stage_index))
        stage_index += 1
    visualize_cell(alpha_network[cell+1][1], network.steps, network.ops['primitives_normal'],  "{}/cell_in_stage_normal_{}".format(folder_path, stage_index))
