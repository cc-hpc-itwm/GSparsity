...
#                                 weight_decay = args.weight_decay)  
    ...
    if args.search_type == "stage":
      steps = 4 # the number of intermediate nodes in a cell
      num_edges = sum([i+2 for i in range(steps)]) #14
      num_ops = len(PRIMITIVES) #7
  ...
  utils_sparsenas.plot_individual_op_norm(model, args.save+"/operator_norm_individual_{}_epoch_{:03d}.png".format(RUN_ID, 0), figsize_width=250)
  if args.search_type == "stage":
  ...
  for epoch in range(1, args.epochs + 1):
    ...
    if args.plot_freq > 0 and epoch % args.plot_freq == 0: #Plot Group sparsity after 10 epochs
      utils_sparsenas.plot_individual_op_norm(model, args.save+"/operator_norm_individual_{}_epoch_{:03d}.png".format(RUN_ID, epoch), figsize_width=250)
      if args.search_type == "stage":
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_decay', type=float, default=40000, help='weight decay (mu)')
    ...
    parser.add_argument('--normalization_exponent', type=float, 
                        default=1, help='normalization exponent to normalize the weight decay (mu)')
    
    parser.add_argument('--save', type=str, default=None, help='experiment name (default None)')
    parser.add_argument('--remark', type=str, default="use cutout", help='remark about the experiment')
