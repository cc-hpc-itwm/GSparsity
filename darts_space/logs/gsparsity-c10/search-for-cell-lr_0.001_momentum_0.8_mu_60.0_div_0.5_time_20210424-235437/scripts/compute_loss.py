...
#         model.load_state_dict(checkpoint['full_weights'])
        ...
        train_loss = np.load("{}/train_objvals.npy".format(args.model_to_resume), allow_pickle=True)
    
    print("param size = {}MB".format(utils.count_parameters_in_MB(model)))
        
    train_reg = utils_sparsenas.get_regularization_term(model_params, args)
    print('mu {} * train_reg {} = {}'.format(args.weight_decay, train_reg.item(), args.weight_decay * train_reg.item()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_decay', type=float, default=50, help='weight decay (mu)')
