...
def get_alpha_from_genotypes(network_eval, genotype):
    num_edges = 8
    num_ops = 7
    step_offset = [0, 2, 5, 9]
    alpha_network = []
    for cell_index in range(network_eval.cells):
        ...
        print("op_names: {}".format(op_names))
        print("indices : {}".format(indices))
        ii = 0
        for op_name, index in zip(op_names, indices):
            op_index = network_eval.ops.index(op_name)
            node_index = ii // 2
            alpha_cell[step_offset[node_index] + index][op_index] = 1
            ii += 1
        
        alpha_network.append((reduce_cell, alpha_cell))
    print(alpha_network)
    return alpha_network
        
# def discretize_search_model_by_operation(model, network_eval, threshold=1e-5, CIFAR_CLASSES = 10):
#   for cell_index, m in enumerate(model.cells):
#         op_index = 0
#         edge_index = 0
...
#                 if "_ops.0" in name: # beginning of a new cell
#                     cur_op_name = name[0:13] # assuming the number of cells < 10
#                     pre_op_name = cur_op_name
#                 else:
#                     if edge_index <= 9:
